...
function renderHospital(hospital) {
    ...
    const insurance = hospital.insurance
        .map((provider) => `<span>${escapeHtml(provider)}</span>`)
        .join(" · ");
    return `
...
function renderNearbyHospital(hospital) {
    const services = hospital.specialties
        .slice(0, 5)
        .map((service) => `<span class="tag">${escapeHtml(service)}</span>`)
        .join("");
...
export function initHospitalsPage() {
    const grid = document.querySelector("#hospitalGrid");
    const search = document.querySelector("#hospitalSearch");
    const district = document.querySelector("#districtFilter");
    const empty = document.querySelector("#noHospitals");
    const resultsCount = document.querySelector("#resultsCount");
    const nearbyHospital = document.querySelector("#nearbyHospital");
